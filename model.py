...
from MODEL.NER.NER_TENSORFLOW.utils import result_to_json
from MODEL.NER.NER_TENSORFLOW.data_utils import create_input, iobes_iob,iob_iobes
...
class Model(object):
    ...
    def embedding_layer(self, char_inputs, seg_inputs, config, name=None):
        ...
        # tf.variable_scope即变量域，与tf.variable的区别是，在variable_scope中可以为tf.get_variable赋予前缀
        # 而tf.get_variable与tf.variable的区别是tf.get_variable可以共享变量，而tf.variable每次都会新建变量
        with tf.variable_scope("char_embedding" if not name else name), tf.device('/gpu:0'):
            self.char_lookup = tf.get_variable(
                    name="char_embedding",
                    # shape为【词汇表长度 ，单个单词的维度数】
...
#tf.nn.atrous_conv2d(value,filters,rate,padding,name=None）
    ...
    def run_step(self, sess, is_train, batch):
        ...
        if is_train:
            global_step, loss,_,char_lookup_out,seg_lookup_out,char_inputs_test,seg_inputs_test,embed_test,embedding_test,\
                ...
                [self.global_step, self.loss, self.train_op,self.char_lookup,self.seg_lookup,self.char_inputs_test,self.seg_inputs_test,self.embed_test,self.embedding_test,self.model_inputs_test,self.layerInput_test,self.conv_test,self.w_test_1,self.w_test_2,self.char_inputs],
                feed_dict)
    ...
    def evaluate_line(self, sess, inputs, id_to_tag):
        ...
        print('string:',inputs[0][0],'tags:',tags)
